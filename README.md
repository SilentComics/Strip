...
* If a comic post is uncategorised it will return an error when `WP_DEBUG` is *true*. The comics are sorted by custom taxonomy “story”; so before adding your own story titles, you should add a “draft” category in the “story” box. From there on if you forget to assign a category to your comic post, comics that are not categorized will then fall as “story draft” without prompting any error.
