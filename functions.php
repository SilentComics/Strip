...
*/
		 ...
		 *
		 * @param array $images filter p tags.
		 ...
		 */
		function filter_ptags_on_images( $images ) {
			$images = preg_replace( '/<p>\s*(<a .*>)?\s*(<img .* \/>)\s*(<\/a>)?\s*<\/p>/iU', '\1\2\3', $images );
			return preg_replace( '/<p>\s*(<a .*>)?\s*(<img .* \/>)\s*(<\/a>)?\s*<\/p>/iU', '\1\2\3', $images );
		}
